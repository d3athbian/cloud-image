...
export class PrefetchQueue {
  ...
  add(url: string, options: PrefetchOptions = {}): boolean {
    ...
    if (existing) {
      if (options.priority !== undefined && options.priority > existing.priority) {
        ...
        this.sortQueue();
        this.notifyListeners({ type: 'queueChange', queueSize: this.queue.length, timestamp: Date.now() });
      }
      return true;
    }
  ...
  private async processTask(task: PrefetchTask): Promise<void> {
    ...
    this.notifyListeners({ type: 'start', task, queueSize: this.queue.length, timestamp: Date.now() });

    try {
      await this.fetchHandler(task.url);
      task.status = 'completed';
      task.completedAt = Date.now();
      this.notifyListeners({ type: 'complete', task, queueSize: this.queue.length, timestamp: Date.now() });
    } catch (error) {
      task.status = 'failed';
      task.error = error instanceof Error ? error : new Error(String(error));
      this.notifyListeners({ type: 'error', task, queueSize: this.queue.length, timestamp: Date.now() });
    }
  }
  ...
  getStats(): { queueSize: number; pending: number; processing: number; completed: number; failed: number } {
    return {
